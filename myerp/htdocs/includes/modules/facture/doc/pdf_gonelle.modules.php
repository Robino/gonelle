...
/**
 *	\file       htdocs/includes/modules/facture/doc/pdf_gonelle.modules.php
 *	\ingroup    facture
 *	\brief      File of class to generate invoices from crab model
 *	\author	    Laurent Destailleur
 *	\version    $Id: pdf_gonelle.modules.php,v 1.12 2011/07/31 23:28:15 eldy Exp $
 */
...
/**
 *	\class      pdf_gonelle
 *	\brief      Classe permettant de generer les factures au modele gonelle
 */

class pdf_gonelle extends ModelePDFFactures
{
	...
	function pdf_gonelle($db)
	{
		...
		$this->db = $db;
		$this->name = "gonelle";
		$this->description = $langs->trans('PDFgonelleDescription');
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0)
	{
		...
		if ($conf->facture->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				{
					// -VMO- Modif des hauteurs
                    $tab_top = 108;

					$pdf->SetFont('Courier','', $default_font_size - 1);
...
//print "$this->marge_gauche, $tab_top-1, $this->page_largeur-$this->marge_gauche-$this->marge_droite, $height_note+1";exit;
				...
				for ($i = 0 ; $i < $nblignes ; $i++)
				{
					...
					$pdf->SetFont('Courier','', $default_font_size - 1);   // Into loop to work with multipage
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs)
	{
		...
		$pdf->SetDrawColor(128,128,128);
        // -VMO- $pdf->SetDrawColor(214,231,183);
	...
	function _pagehead(&$pdf, $object, $showaddress=1, $outputlangs)
	{
		...
		if ($this->emetteur->logo)
		{
			...
			{
				// -VMO-
                $pdf->Image($logo, 0, 0, 210, 0);	// width=0 (auto), max height=24
			}
			else
			{
				$pdf->SetTextColor(200,0,0);
	    ...
	    foreach($object->linkedObjects as $objecttype => $objects)
	    {
			...
			else if ($objecttype == 'commande')
			{
	    		...
	    		for ($i=0;$i<$num;$i++)
	    		{
	    			...
	    			//vmo : on ajoute la référence du devis venant de l'objet lié à la commande
	    			$vmoobjet = $objects[$i];
	    			$vmoobjet->fetchObjectLinked();
	    			foreach ($vmoobjet->linkedObjects as $vmotype => $objectpropal)
	    			{
	    				if ($vmotype == 'propal')
	    				{
	    					$vmotext=$outputlangs->transnoentities($objectpropal[0]->ref);
	    					$pdf->MultiCell(100, 3, $outputlangs->transnoentities("RefProposal")." : ".$outputlangs->transnoentities($vmotext), '', 'R');
	    					$posy+=4;
	    				}
	    			}
	    			//fin de l'ajout de la référence devis
	    			
	    			if ($objects[$i]->ref_client) $text.=' ('.$objects[$i]->ref_client.')';
	    			$pdf->SetXY(100,$posy); //vmo ajout de l'axe des x suite à l'ajout de la référence des devis
		...
		if ($showaddress)
		{
			...
			// -VMO- $carac_client=pdf_build_address($outputlangs,$this->emetteur,$object->client,$object->contact,$usecontact,'target');
			...
			// -VMO- $pdf->MultiCell(80,5, $outputlangs->transnoentities("BillTo").":",0,'L');
	...
	function _pagefoot(&$pdf,$object,$outputlangs)
	{
		...
		// Logo
		//$basdepage='basdepage2.jpg';
		//$pdf->Image($basdepage, $this->marge_gauche, 100, 0, 24);	// width=0 (auto), max height=24
        // Logo
		$logo=$conf->mycompany->dir_output.'/logos/basdepage2facture.jpg';
