...
/**
 *	\file       htdocs/includes/modules/propale/pdf_propale_gonelle.modules.php
 *	\ingroup    propale
 *	\brief      Fichier de la classe permettant de generer les propales au modele gonelle
 *	\author	    Laurent Destailleur
 *	\version    $Id: pdf_propale_gonelle.modules.php,v 1.243 2011/07/31 23:28:16 eldy Exp $
 */
...
/**
 *	\class      pdf_propale_gonelle
 *	\brief      Classe permettant de generer les propales au modele gonelle
 */
class pdf_propale_gonelle extends ModelePDFPropales
{
	 ...
	 */
	function pdf_propale_gonelle($db)
	{
		...
		$this->db = $db;
		$this->name = "gonelle";
		$this->description = $langs->trans('DocModelgonelleDescription');
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0)
	{
		...
		if ($conf->propale->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				$tab_top_newpage = 60;
				$tab_height = 110; // -VMO- pour support du logo bas
				$tab_height_middlepage = 200;
				$tab_height_endpage = 170;

				// Affiche notes VMO (si c'est un particulier on affiche même si la note est vide
				if (! empty($object->note_public) || ! $object->client->isACompany() )
				{
					$tab_top = 108; //-VMO-

					$pdf->SetFont('Courier','', $default_font_size - 1);   // Dans boucle pour gerer multi-page
					$pdf->SetXY ($this->posxdesc-1, $tab_top);
					// VMO - On ajoute fonction du type de société particulier ou entreprise 
					if ($object->client->isACompany())
					{
						$vmo_info_note=$object->note_public;
					}
					else
					{
						$vmo_info_note=$object->note_public.'30% d\'acompte à la commande et le solde à la réception des travaux';
					}
					
					$pdf->MultiCell(190, 4, $outputlangs->convToOutputCharset($vmo_info_note), 0, 'L');
					$nexY = $pdf->GetY();
				...
				for ($i = 0 ; $i < $nblignes ; $i++)
				{
					...
					$pdf->SetFont('Courier','', $default_font_size - 1);   // On repositionne la police par defaut
					$nexY = $pdf->GetY();
				...
				// VMO affiche Bon pour commande
				$pdf->SetXY($this->marge_gauche,-55);
    			$pdf->SetFont('','U','');
    			$pdf->Cell(0,10,'Bon pour accord (date,signature,cachet)',0,0,'R');
	...
	function _pagehead(&$pdf, $object, $showaddress=1, $outputlangs)
	{
		...
		// Logo
		$logo=$conf->mycompany->dir_output.'/logos/hautdepage.jpg';
		if ($this->emetteur->logo)
		{
			if (is_readable($logo))
			{
				$pdf->Image($logo, 0, 0, 210, 0);
			}
			else
			{
				$pdf->SetTextColor(200,0,0);
		...
		if ($showaddress)
		{
			...
			// -VMO- $pdf->MultiCell(66,5, $outputlangs->transnoentities("BillFrom").":", 0, 'L');
			$pdf->SetXY($posx,$posy);
			...
			{					
					//VMO - Tests si la compagnie est un particulier ou une société
					if ($object->client->isACompany())
					{
			...
			$pdf->SetFont('','', $default_font_size - 2);
			$pdf->SetXY($posx,$posy-5);
			// -VMO- $pdf->MultiCell(80, 4, $outputlangs->transnoentities("BillTo").":", 0, 'L');
	...
	function _pagefoot(&$pdf,$object,$outputlangs)
	{
		...
		$logo=$conf->mycompany->dir_output.'/logos/basdepage2.png';
		$pdf->Image($logo, 0, 253, 210, 0);	// width=0 (auto), max height
